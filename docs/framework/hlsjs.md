...
function VideoPlayer({ src, poster }) {
  ...
  return (
    <div className='video-player'>
      <video ref={videoRef} controls poster={poster} playsInline crossOrigin='anonymous' />
    </div>
  )
}
...
function App() {
  ...
  return (
    <div className='app'>
      ...
      <VideoPlayer src={videoUrl} poster='https://example.com/path/to/poster.jpg' />
    </div>
  )
}
...
function QualitySelector({ hls, levels }) {
  ...
  return (
    <div className='quality-selector'>
      <select value={currentLevel} onChange={(e) => handleLevelChange(parseInt(e.target.value))}>
...
function AudioTrackSelector({ hls, audioTracks }) {
  ...
  return (
    <div className='audio-selector'>
      <select value={currentAudio} onChange={(e) => handleAudioChange(parseInt(e.target.value))}>
...
function LivePlayer({ src, enableDvr }) {
  const { videoRef, isPlaying, togglePlay, currentTime, duration, seek, loading, error } = useHlsPlayer(src)
        ...
        {enableDvr && isDvrAvailable && (
          <>
            <div className='dvr-controls'>
              <span className='time'>{Math.max(0, Math.floor(currentTime - duration))}s (DVR)</span>
              <input
                type='range'
                min={0}
                max={duration}
...
function CustomVideoPlayer({ src, poster }) {
  ...
  return (
    <div className='video-container'>
      <video ref={videoRef} poster={poster} playsInline crossOrigin='anonymous' className='video-player' />
      ...
      <div className='video-overlay'>
        ...
        <div className='extra-controls'>
          {levels.length > 0 && <QualitySelector levels={levels} />}

          {audioTracks.length > 0 && <AudioTrackSelector audioTracks={audioTracks} />}

          {subtitleTracks.length > 0 && <SubtitleSelector subtitleTracks={subtitleTracks} />}
        </div>
      </div>
    </div>
  )
    ...
    <div className='video-portal'>
      ...
      <div className='portal-content'>
        <div className='main-player'>
          {selectedVideo ? (
            <CustomVideoPlayer src={selectedVideo.url} poster={selectedVideo.thumbnail} />
          ) : (
  ...
  return (
    <div className='education-player'>
      <div className='player-section'>
        {lesson && <CustomVideoPlayer src={lesson.videoUrl} poster={lesson.thumbnail} />}
      </div>
      ...
      <div className='content-section'>
        ...
        <div className='lesson-navigation'>
          <button disabled={currentLesson === 0} onClick={() => setCurrentLesson((prev) => prev - 1)}>
            Previous Lesson
          </button>
          <span>
