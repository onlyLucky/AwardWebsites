...
import { headerStyles } from '@/demos/animejs/styles'
...
export default function Header({ isHome = true }: HeaderProps) {
  ...
  return (
    <header
      id='site-header'
      className={headerStyles['site-header']}
      style={{
    ...
    >
      <div
        ...
        className={headerStyles['site-header-content']}
        style={{
          borderBottom: borderBottom,
        }}
      >
        {/* Logo 区域 */}
        <h1 className={`heading-logo header-logo ${headerStyles['header-logo']}`}>
          <a href='/'>
        ...
        <nav
          id='site-menu'
          className={`${headerStyles['site-menu']}${menuActive ? ` ${headerStyles['is-active']} is-active` : ''}`}
        >
          <ul>
            {/* Docs */}
            <li>
              <a
                className={`main-nav-link docs-link ${headerStyles['main-nav-link']}`}
                data-id='docs'
                href='documentation.html'
                title='Documentation'
              >
                <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' className={headerStyles.icon}>
            ...
            <li>
              <a
                className={`main-nav-link ${headerStyles['main-nav-link']}`}
                href='easing-editor.html'
                title='Easing functions editor'
              >
                <svg xmlns='http://www.w3.org/2000/svg' width='24' height='24' viewBox='0 0 24 24' className={headerStyles.icon}>
            ...
            <li>
              <a
                className={`main-nav-link nav-link-retractable ${headerStyles['main-nav-link']} ${headerStyles['nav-link-retractable']}`}
            ...
            {/* Sponsor - 带有特殊背景效果 */}
            <li className={headerStyles['sponsor-item']}>
              <SponsorButton>
                <a
                  className={`main-nav-link ${headerStyles['main-nav-link']} ${headerStyles['sponsor-link']}`}
                  href='https://github.com/sponsors/juliangarnier'
                  target='_blank'
                  title='Sponsor Julian Garnier on GitHub'
                >
                  {/* 桌面端赞助按钮背景 */}
                  <span className={headerStyles['sponsor-link-bg']} />
                  <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' className={headerStyles.icon}>
                    <g fill='none' fillRule='evenodd'>
        ...
        <button
          id='toggle-site-menu'
          className={headerStyles['toggle-site-menu']}
